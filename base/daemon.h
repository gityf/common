...
*/
#ifndef _COMMON_DEAMON_H_
#define _COMMON_DEAMON_H_
#include <fcntl.h>
#include <unistd.h>
#include <cstdio>
#include <cstdlib>
namespace common {
    ...
    int DaemonMode() {
        ...
        if (freopen("/dev/null", "r", stdin) == 0) {
            fprintf(stderr, ":Cannot replace stdin with /dev/null: %s.\n",
                strerror(errno));
        }
        if (freopen("/dev/null", "w", stdout) == 0) {
            fprintf(stderr, ":Cannot replace stdout with /dev/null: %s.\n",
                strerror(errno));
        }
        if (freopen("/dev/null", "w", stderr) == 0) {
            fprintf(stderr, ":Cannot replace stderr with /dev/null: %s.\n",
                strerror(errno));
        }
        //*/
        return 0;
