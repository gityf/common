...
**Description: Source file of class Inotify.
*/
...
void Inotify::loop(long waitMs) {
    ...
    while (!isStoped_) {
        struct inotify_event *ev = inotify_next_events(waitMs, 2);
        if (ev != NULL) {
            string tfile = fd2files[ev->wd];
            if (fileWatchs_.find(tfile) !=
                fileWatchs_.end()) {
                if (FileSys::IsDir(tfile)) {
                    // tfile is directory and sub file changed in this directory.
                    string fullFfile = tfile + "/" + ev->name;
                    fileWatchs_[tfile](ev->mask, fullFfile);
                }
                else {
