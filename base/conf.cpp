...
std::list<string> Conf::getStrings(const string& section, const string& name) {
    string key = makeKey(section, name);
    auto p = values_.find(key);
    return p == values_.end() ? std::list<string>() : p->second;
}
...
int Conf::parse(const string& filename1) {
    filename_.assign(filename1.data(), filename1.length());
