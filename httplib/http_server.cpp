...
int HttpEpollWatcher::handle_request(Request &req, Response &res) {
	...
	if(this->resource_map.find(uri) == this->resource_map.end()) { // not found
		...
		log_fatal(STATUS_NOT_FOUND.status_code, "page not found which uri:" << uri);
		return 0;
	}
	...
	if(method.name != req.line.method) {
		...
		res.body.clear();
		log_warn(STATUS_METHOD_NOT_ALLOWED.status_code, "not allow method, allowed:"<< method.name << "request method:" << req.line.method);
		return 0;
	}
	...
	log_debug("handle response success which code:" << res.code_msg.status_code << "msg:" << res.code_msg.msg);
	return 0;
}
...
int HttpEpollWatcher::on_writeable(EpollContext &epoll_context) {
	...
	log_debug("send complete which write_num:" << nwrite << ",read_size:" << read_size);
