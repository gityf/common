...
public:
	...
	void get_all_params(std::map<std::string, std::string> &param_values);
...
public:
	...
	void print_access_log(std::string &client_ip) {
		log_warn(ERRNO_OK, "access_log " << this->req->line.method << " "
							   << this->req->line.request_url
							   << " status_code:" << res->code_msg.status_code
							   << "cost_time:" << get_cost_time()
							   << " us, body_size:" << res->body.size()
							   <<",client_ip:" << client_ip);
	}
